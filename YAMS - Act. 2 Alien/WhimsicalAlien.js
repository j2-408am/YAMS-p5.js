function setup() {
  createCanvas(515, 500);
  background("#150D29"); // Dark space-like background

  // Outer sci-fi frame
  fill("#140644");
  stroke("#140644");
  rect(10, 10, 500, 450);

  // Inner sci-fi frame (gives a bordered screen effect)
  fill("#08145E");
  stroke("#08145E");
  rect(30, 30, 450, 400);

  // Stars
  fill("yellow");
  stroke("#FFFF00");
  strokeWeight(2);
  ellipse(25, 35, 1, 20);
  ellipse(25, 35, 20, 1);
  ellipse(490, 400, 1, 20);
  ellipse(490, 400, 20, 1);
  ellipse(75, 275, 1, 20);
  ellipse(75, 275, 20, 1);
  ellipse(350, 235, 1, 20);
  ellipse(350, 235, 20, 1);
  ellipse(420, 65, 1, 20);
  ellipse(420, 65, 20, 1);

  // UFO
  fill("#B1C8F2");
  noStroke();
  ellipse(413, 260, 35, 35); // Dome
  fill("red");
  ellipse(413, 269, 60, 20); // Base
  fill("yellow");
  ellipse(413, 269, 5, 5); // Lights
  ellipse(400, 269, 5, 5);
  ellipse(426, 269, 5, 5);

  // Planetary base or moon surface below
  fill("rgb(242,237,207)");
  stroke("#A3A3B2");
  strokeWeight(10);
  ellipse(253, 700, 700, 700); // Giant ellipse for moon or planet

  // Face Features
  fill("#EADBBE");
  noStroke();
  ellipse(255, 73, 50, 50);
  ellipse(380, 73, 50, 50);

  // Inner eyes (yellow glow)
  fill("#E0C442");
  ellipse(255, 73, 40, 40);
  ellipse(380, 73, 40, 40);

  // Antenna
  fill("#475F2B");
  stroke("#475F2B");
  strokeWeight(2);
  rect(250, 75, 10, 50);
  rect(375, 75, 10, 50);

  // Wings
  fill("#F9A700");
  stroke("#276529");
  strokeWeight(3);
  ellipse(255, 75, 30, 30);
  ellipse(380, 75, 30, 30);

  // Cat head shape using bezier curves
  translate(width / 1 - 325, 100);
  beginShape();
  vertex(12, 350);
  fill("#E1ABAB");
  stroke("#FF0033");
  strokeWeight(7);
  bezierVertex(-250, 45, 120, 150, -30, 0);
  bezierVertex(50, 18, 350, 40, 150, 0);
  bezierVertex(36, 15, 35, 25, 20, 100);
  vertex(120, 300);
  vertex(-20, 150);
  endShape();

  // Head shadow or dimension effect
  noStroke();
  ellipse(145, 58, 257, 100);

  // Cat face base
  fill("#F0D49B");
  ellipse(147, 67, 100, 75);

  // Alien antenna base
  scale(1, 2);
  fill("#ACD928");
  stroke("#475F2B");
  strokeWeight(2);
  ellipse(147, 27, 20, 5); // top antenna

  // Left eye (white + black pupil)
  push();
  translate(77, 35);
  rotate(radians(-60));
  fill("white");
  stroke("black");
  strokeWeight(3);
  ellipse(10, 0, 25, 40);
  pop();

  push();
  translate(77, 33);
  rotate(radians(-60));
  fill("black");
  stroke("black");
  strokeWeight(3);
  ellipse(10, 0, 17, 35);
  pop();

  // Right eye
  push();
  translate(205, 17);
  rotate(radians(60));
  fill("white");
  stroke("black");
  strokeWeight(3);
  ellipse(10, 0, 25, 40);
  pop();

  push();
  translate(205, 15);
  rotate(radians(60));
  fill("black");
  stroke("black");
  strokeWeight(3);
  ellipse(10, 0, 17, 35);
  pop();

  // Nose
  push();
  fill("#F0D49B");
  noStroke();
  translate(25, -5);
  triangle(-55, 5, -5, 15, -1, 30);
  pop();

  // Mouth
  push();
  translate(140, 21);
  rotate(radians(60));
  fill("black");
  noStroke();
  ellipse(10, 0, 3, 17);
  pop();

  // Mouth
  push();
  translate(142, 38);
  rotate(radians(-60));
  fill("black");
  noStroke();
  ellipse(10, 0, 3, 17);
  pop();

  // Antenna bulb
  fill("#F9A700");
  stroke("#276529");
  strokeWeight(1);
  ellipse(147, 27, 20, 5);

  // Middle Eye
  fill("white");
  stroke("black");
  strokeWeight(2);
  ellipse(143, 11, 50, 13);

  // Iris
  fill("white");
  stroke("white");
  strokeWeight(2);
  ellipse(80, 22, 10, 10);
  ellipse(210, 22, 10, 10);

  // Inner Eye (Middle)
  fill("black");
  stroke("black");
  strokeWeight(2);
  ellipse(143, 11, 35, 10);
  fill("#FFFFFF");
  stroke("#FFFFFF");
  ellipse(143, 10, 7, 7);

  // Left ear (rotated ellipse)
  translate(-185, -23);
  rotate(radians(-50));
  fill("#F9A700");
  stroke("#276529");
  strokeWeight(4);
  ellipse(5, 100, 25, 85);

  // Right ear
  translate(75, 25);
  rotate(radians(30));
  fill("#F9A700");
  stroke("#276529");
  strokeWeight(4);
  ellipse(5, 100, 25, 85);
}